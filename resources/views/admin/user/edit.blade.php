                    ...
                    <form ref="myForm" autocomplete="off">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label>{{ __('UID') }}<span class="red">*</span></label>
                                <input type="text" class="form-control" v-model="editData.uid" ref="uid" placeholder="{{ __('UID') }}" readonly>
                            </div>
                            <div class="form-group col-md-6">
                                <label>{{ __('新密碼') }}<span class="red" v-if="id==0">*</span></label>
                                <input type="password" class="form-control" v-model="editData.password" ref="password" placeholder="{{ __('新密碼，無變動不需填寫') }}" autocomplete="new-password">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label>{{ __('帳號') }}<span class="red">*</span></label>
                                <input type="text" class="form-control" v-model="editData.account" ref="account" placeholder="{{ __('帳號') }}">
                            </div>
                            <div class="form-group col-md-6">
                                <label>{{ __('玩家遊戲名稱') }}<span class="red"></span></label>
                                <input type="text" class="form-control" v-model="editData.name" ref="name" placeholder="{{ __('玩家遊戲名稱') }}">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label>{{ __('可建地編圖數量限制') }}</label>
                                <input type="number" class="form-control" v-model="editData.map_limit" ref="map_limit" placeholder="{{ __('可建地編圖數量限制') }}">
                            </div>
                            <!-- <div class="form-group col-md-6">
...
<script type="text/javascript">
    var vc = new Vue({
        ...
        data:{
            ...
            admin_roles     : [],
            words           : {
                "type" : "{{ $id?__('玩家編輯'):__('玩家新增') }}",
                "data_warning" : '',
            },
        },
        methods: {
            get() {
                let url = "{{ config('services.API_URL').'/user' }}/"+this.id;
                $.ajax({
                    method: "GET",
                    url: url,
                    data: {},
...
console.log(vc.editData);
            ...
            checkSave(){
                ...
                if(vc.id >= 1){
                    method = "PUT";
                    url = "{{ config('services.API_URL').'/user' }}/"+this.id;
                }
