...
<div class="container-fluid" id="vc">
    <div class="row">
        ...
        <div class="top-right-button-container mb-2">
            <a href="{{ route('store.employee.role.edit',0) }}" class="btn btn-primary badge-primary-color top-right-button mr-1">{{ __('default.新增') }}</a>
        </div>
        @stop
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    <th scope="col">#</th>
                                    <th scope="col" @click="get('sort', 'store_employee_roles.name')">{{ __('default.身分') }}</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
...
<script type="text/javascript">
    var vc = new Vue({
        ...
        methods: {
            ...
            checkSave(editData, is_active = 1){
                Swal.fire({
                    ...
                    if(result.isConfirmed){
                        method = "PUT";
                        url = "{{ config('services.API_URL').'/store_employee_role' }}/"+editData.id;
            ...
            goEdit(id){
                location.href = '{{ route('store.employee.role.edit') }}/'+id
            }
        },
        created : function(){
            this.get();
