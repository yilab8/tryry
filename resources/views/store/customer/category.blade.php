                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    <th scope="col">#</th>
                                    <th scope="col" @click="get('sort','store_customer_category.name')">{{ __('default.類別') }}</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
    ...
    var vc = new Vue({
        ...
        methods: {
            ...
            get(type = false, sort = false) {
                ...
                vc = this;
                let url = "{{ config('services.API_URL').'/store_customer_category' }}"
                $.ajax({
                    method: "GET",
                    url: url,
                    data: vc.searchData,
            ...
            openDataModal(obj = false){
                if(obj===false){
                    ...
                    vc.editData = {'id':0, 'name':'', 'is_active': 1, 'keepalive': 0};
                }
                else{
            ...
            checkSave(){
                ...
                if(vc.editData.id >= 1){
                    method = "PUT";
                    url = "{{ config('services.API_URL').'/store_customer_category' }}/"+vc.editData.id;
                }
            ...
            checkSortSave(){
                let method = "POST";
                let url = "{{ config('services.API_URL').'/store_customer_category/update_sort' }}";
