...
<div class="container-fluid" id="vc">
    <div class="row">
        ...
        <div class="modal fade modal-bottom" id="dataModal" tabindex="-1" role="dialog" aria-labelledby="dataModal" aria-hidden="true">
                    ...
                    <div class="modal-body" v-if="editData">
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label>{{ __('default.名稱') }}<span class="red">*</span></label>
                                <input type="text" class="form-control" v-model="editData.name" placeholder="">
                            </div>
                            <div class="form-group col-md-6">
                                <label>{{ __('default.商業登記號碼') }}</label>
                                <input type="text" class="form-control" v-model="editData.tax_id" placeholder="">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label>{{ __('default.聯絡人') }}</label>
                                <input type="text" class="form-control" v-model="editData.contact_name" placeholder="">
                            </div>
                            <div class="form-group col-md-6">
                                <label>{{ __('default.聯絡電話') }}</label>
                                <input type="tel" class="form-control" v-model="editData.contact_phone" placeholder="">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <label>{{ __('default.地址') }}</label>
                                <input type="text" class="form-control" v-model="editData.address" placeholder="">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    <th scope="col">#</th>
                                    <th scope="col" @click="get('sort','store_product_vendor.name')">{{ __('default.名稱') }}</th>
                                    <th scope="col" @click="get('sort','store_product_vendor.name')">{{ __('default.聯絡人') }}</th>
                                    <th scope="col" @click="get('sort','store_product_vendor.name')">{{ __('default.聯絡電話') }}</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(list,i) in lists" @click.prevent="openDataModal(list)">
                                    <td scope="row">@{{ i+1 }}</td>
                                    <td scope="row">@{{ list.name }}</td>
                                    <td scope="row">@{{ list.contact_name }}</td>
                                    <td scope="row">@{{ list.contact_phone }}</td>
    ...
    var vc = new Vue({
        ...
        methods: {
            ...
            get(type = false, sort = false) {
                ...
                vc = this;
                let url = "{{ config('services.API_URL').'/store_product_vendor' }}"
                $.ajax({
                    method: "GET",
                    url: url,
                    data: vc.searchData,
            ...
            checkSave(){
                if(vc.editData.name==''){
                    vc.words.data_warning = "{{ __('default.請輸入名稱') }}";
                    return false;
                }
                let method = "POST";
                let url = "{{ config('services.API_URL').'/store_product_vendor' }}";
                ...
                if(vc.editData.id >= 1){
                    method = "PUT";
                    url = "{{ config('services.API_URL').'/store_product_vendor' }}/"+vc.editData.id;
                }
            ...
            checkSortSave(){
                let method = "POST";
                let url = "{{ config('services.API_URL').'/store_product_vendor/update_sort' }}";
