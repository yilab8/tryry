                ...
                <div class="card-body">
                    <div class="form-row">
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" class="form-control" v-model="searchData.store_employees__name" placeholder="{{ __('default.姓名') }}" @keyup.enter="get()">
                        </div>
                        <div class="form-group col-md-3">
                            <label>{{ __('default.身分') }}</label>
                            <select v-model="searchData.store_employees__store_employee_role_id" id="store_employee_role_id" class="form-control select2-single" onChange="vc.get()">
                                <option value="0">{{ __('default.請選擇') }}</option>
                                <option v-for="(store_employee_role, i) in store_employee_roles" :value="store_employee_role.id">@{{ store_employee_role.name }}</option>
                            </select>
                        </div>
                        <div class="form-group col-md-3">
                            <label>{{ __('default.帳號') }}</label>
                            <input type="text" class="form-control" v-model="searchData.store_employees__account" placeholder="{{ __('default.帳號') }}" @keyup.enter="get()">
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    <th scope="col">#</th>
                                    <th scope="col" @click="get('sort', 'store_employees.name')">{{ __('default.姓名') }}</th>
                                    <th scope="col" @click="get('sort', 'store_employees.account')">{{ __('default.帳號') }}</th>
                                    <th scope="col" @click="get('sort', 'store_employees.cellphone')">{{ __('default.聯絡電話') }}</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(list,i) in lists" @click="goEdit(list.id)">
                                    ...
                                    <td scope="row">@{{ list.cellphone }}</td>
    ...
    var vc = new Vue({
        ...
        data:{
            ...
            searchData  : {
                ...
                'store_employees__store_employee_role_id' : 0,
                'store_employees__account' : '',
                'store_employees__name' : '',
                'sort'      : 'sort',
                'direction' : 'asc',
            },
            sorts       : [],
        ...
        methods: {
            get(type = false, sort = false) {
                ...
                this.searchData.store_employees__store_employee_role_id= $('#store_employee_role_id').val();
console.log(this.searchData);
            ...
            getStoreEmployeeRoles(){
                let url = "{{ config('services.API_URL').'/store_employee_role' }}";
                $.ajax({
                    method: "GET",
                    url: url,
                    data: {
                        'store_employee_roles__store_id' : '{{ auth()->guard("store")->user()->store_id }}',
                    },
                    dataType: 'json',
                    success(data){
                        vc.store_employee_roles = data.data.data;
                    },
                    error:function(xhr, ajaxOptions, thrownError){
                        console.log(xhr);
            ...
            checkSave(editData, is_active = 1){
                Swal.fire({
                    ...
                    if(result.isConfirmed){
                        method = "PUT";
                        url = "{{ config('services.API_URL').'/store_employee' }}/"+editData.id;
            ...
            checkSortSave(){
                let method = "POST";
                let url = "{{ config('services.API_URL').'/store_employee/update_sort' }}";
            ...
            goEdit(id){
                location.href = '{{ route('store.employee.edit') }}/'+id
            }
        },
        created : function(){
            this.get();
            this.getStoreEmployeeRoles();
        },
        mounted: function(){
            var self = this;
