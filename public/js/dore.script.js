...
$.dore = function (element, options) {
  ...
  function init() {
    ...
    /* 03.12. Notification */
    function showNotification(message = '', placementFrom = 'top', placementAlign = 'center', type = 'primary') {
      $.notify(
        {
    ...
    $("body").on("click", ".notify-btn", function (event) {
      event.preventDefault();
      showNotification($(this).data("message"), $(this).data("from"), $(this).data("align"), "primary");
    });
    ...
    if (typeof Glide !== "undefined") {
      ...
      if ($(".glide.dashboard-numbers").length > 0) {
        new Glide(".glide.dashboard-numbers", {
            ...
            startAt: 0,
            perView: 6,
            perTouch: 6, // 將 perTouch 設置為 perView 的值
            gap: 7,
            breakpoints: {
                1800: {
                    perView: 6
